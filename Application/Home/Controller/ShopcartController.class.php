...
$shoplist=$goodlist->where("orderid='$orderid'")->select();
	  ...
	  //获取地址信息数据,数据表transport字段address
	  $msg=get_address($uid);
...
$this->assign('codeid',$ordercode);
 // 配置邮件提醒   
	 $mail=get_email($ordercode);//获取会员ucenter注册邮箱,字段email
	 $title="订单提交提醒";
	 $content="您在<a href='".C('DOMAIN')."' target='_blank'>".C('SITENAME').'</a>支付了订单，订单号'.$ordercode;
